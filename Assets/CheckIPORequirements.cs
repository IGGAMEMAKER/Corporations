    ...
    , IAnyDateListener
{
    ...
    void Render()
    {
        ...
        Hint.SetHint($"Requirements" +
            Visuals.Colorize($"\nCompany Cost more than ${Format.Shorten(Constants.IPO_REQUIREMENTS_COMPANY_COST)}", CompanyUtils.IsMeetsIPOCompanyCostRequirement(GameContext, companyId))  +
            Visuals.Colorize($"\nMore than 3 shareholders", CompanyUtils.IsMeetsIPOShareholderRequirement(GameContext, companyId)) + 
            Visuals.Colorize($"\nProfit bigger than ${Format.Shorten(Constants.IPO_REQUIREMENTS_COMPANY_PROFIT)}", CompanyUtils.IsMeetsIPOProfitRequirement(GameContext, companyId))
            );
