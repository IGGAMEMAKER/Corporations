//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.CodeGeneration.Plugins.ComponentLookupGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public static class GameComponentsLookup {

    public const int AcceptsInvestments = 0;
    public const int AnyCompanyListener = 1;
    public const int AnyDateListener = 2;
    public const int AnyNotificationsListener = 3;
    public const int AnyShareholdersListener = 4;
    public const int AnyTechnologyLeaderListener = 5;
    public const int CEO = 6;
    public const int Company = 7;
    public const int CompanyGoal = 8;
    public const int CompanyListener = 9;
    public const int CompanyPhase = 10;
    public const int CompanyResource = 11;
    public const int CompanyResourceListener = 12;
    public const int ControlledByPlayer = 13;
    public const int Cooldowns = 14;
    public const int Crunching = 15;
    public const int CrunchingListener = 16;
    public const int Date = 17;
    public const int DevelopmentFocus = 18;
    public const int DevelopmentFocusListener = 19;
    public const int EventFinancePricingChange = 20;
    public const int EventMarketingEnableTargeting = 21;
    public const int EventMarketingStartSimpleCampaign = 22;
    public const int EventStaffHireProgrammer = 23;
    public const int EventUpgradeAnalytics = 24;
    public const int EventUpgradeProduct = 25;
    public const int Finance = 26;
    public const int FinanceListener = 27;
    public const int Following = 28;
    public const int Human = 29;
    public const int IndependentCompany = 30;
    public const int Industry = 31;
    public const int InvestmentProposals = 32;
    public const int InvestmentRounds = 33;
    public const int Marketing = 34;
    public const int MarketingListener = 35;
    public const int Menu = 36;
    public const int MenuListener = 37;
    public const int MetricsHistory = 38;
    public const int NavigationHistory = 39;
    public const int Niche = 40;
    public const int NicheCosts = 41;
    public const int NicheState = 42;
    public const int Notifications = 43;
    public const int Product = 44;
    public const int ProductListener = 45;
    public const int PublicCompany = 46;
    public const int Shareholder = 47;
    public const int Shareholders = 48;
    public const int ShareholdersListener = 49;
    public const int Targeting = 50;
    public const int TargetingListener = 51;
    public const int TargetUserType = 52;
    public const int TargetUserTypeListener = 53;
    public const int Task = 54;
    public const int TaskManager = 55;
    public const int Team = 56;
    public const int TeamListener = 57;
    public const int TechnologyLeader = 58;
    public const int TechnologyLeaderListener = 59;
    public const int TimerRunning = 60;

    public const int TotalComponents = 61;

    public static readonly string[] componentNames = {
        "AcceptsInvestments",
        "AnyCompanyListener",
        "AnyDateListener",
        "AnyNotificationsListener",
        "AnyShareholdersListener",
        "AnyTechnologyLeaderListener",
        "CEO",
        "Company",
        "CompanyGoal",
        "CompanyListener",
        "CompanyPhase",
        "CompanyResource",
        "CompanyResourceListener",
        "ControlledByPlayer",
        "Cooldowns",
        "Crunching",
        "CrunchingListener",
        "Date",
        "DevelopmentFocus",
        "DevelopmentFocusListener",
        "EventFinancePricingChange",
        "EventMarketingEnableTargeting",
        "EventMarketingStartSimpleCampaign",
        "EventStaffHireProgrammer",
        "EventUpgradeAnalytics",
        "EventUpgradeProduct",
        "Finance",
        "FinanceListener",
        "Following",
        "Human",
        "IndependentCompany",
        "Industry",
        "InvestmentProposals",
        "InvestmentRounds",
        "Marketing",
        "MarketingListener",
        "Menu",
        "MenuListener",
        "MetricsHistory",
        "NavigationHistory",
        "Niche",
        "NicheCosts",
        "NicheState",
        "Notifications",
        "Product",
        "ProductListener",
        "PublicCompany",
        "Shareholder",
        "Shareholders",
        "ShareholdersListener",
        "Targeting",
        "TargetingListener",
        "TargetUserType",
        "TargetUserTypeListener",
        "Task",
        "TaskManager",
        "Team",
        "TeamListener",
        "TechnologyLeader",
        "TechnologyLeaderListener",
        "TimerRunning"
    };

    public static readonly System.Type[] componentTypes = {
        typeof(AcceptsInvestmentsComponent),
        typeof(AnyCompanyListenerComponent),
        typeof(AnyDateListenerComponent),
        typeof(AnyNotificationsListenerComponent),
        typeof(AnyShareholdersListenerComponent),
        typeof(AnyTechnologyLeaderListenerComponent),
        typeof(CEOComponent),
        typeof(CompanyComponent),
        typeof(CompanyGoalComponent),
        typeof(CompanyListenerComponent),
        typeof(CompanyPhaseComponent),
        typeof(CompanyResourceComponent),
        typeof(CompanyResourceListenerComponent),
        typeof(ControlledByPlayerComponent),
        typeof(CooldownsComponent),
        typeof(CrunchingComponent),
        typeof(CrunchingListenerComponent),
        typeof(DateComponent),
        typeof(DevelopmentFocusComponent),
        typeof(DevelopmentFocusListenerComponent),
        typeof(EventFinancePricingChangeComponent),
        typeof(EventMarketingEnableTargetingComponent),
        typeof(EventMarketingStartSimpleCampaign),
        typeof(EventStaffHireProgrammerComponent),
        typeof(EventUpgradeAnalyticsComponent),
        typeof(EventUpgradeProductComponent),
        typeof(FinanceComponent),
        typeof(FinanceListenerComponent),
        typeof(FollowingComponent),
        typeof(HumanComponent),
        typeof(IndependentCompanyComponent),
        typeof(IndustryComponent),
        typeof(InvestmentProposalsComponent),
        typeof(InvestmentRoundsComponent),
        typeof(MarketingComponent),
        typeof(MarketingListenerComponent),
        typeof(MenuComponent),
        typeof(MenuListenerComponent),
        typeof(MetricsHistoryComponent),
        typeof(NavigationHistoryComponent),
        typeof(NicheComponent),
        typeof(NicheCostsComponent),
        typeof(NicheStateComponent),
        typeof(NotificationsComponent),
        typeof(ProductComponent),
        typeof(ProductListenerComponent),
        typeof(PublicCompanyComponent),
        typeof(ShareholderComponent),
        typeof(ShareholdersComponent),
        typeof(ShareholdersListenerComponent),
        typeof(TargetingComponent),
        typeof(TargetingListenerComponent),
        typeof(TargetUserTypeComponent),
        typeof(TargetUserTypeListenerComponent),
        typeof(TaskComponent),
        typeof(TaskManagerComponent),
        typeof(TeamComponent),
        typeof(TeamListenerComponent),
        typeof(TechnologyLeaderComponent),
        typeof(TechnologyLeaderListenerComponent),
        typeof(TimerRunningComponent)
    };
}
