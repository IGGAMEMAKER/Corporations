...
public class RenderClientsDataForProduct : View
{
    public override void ViewRender()
    {
        ...
        for (var i = startDate; i < endDate; i+= (int)C.PERIOD) // 7 - period length
            xs.Add(i);
    ...
    List<long> NormalizeGraphData(GameEntity p, int start, int end, int necessaryMetrics)
    {
        ...
        {
            metrics.Add(GetMetricMockup(0, start + i * (int)C.PERIOD));
        }

        // add current value
        metrics.Add(GetMetricMockup(Marketing.GetUsers(p), CurrentIntDate + 1));
